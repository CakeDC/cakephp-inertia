...
class SampleTemplateCommand extends \Bake\Command\TemplateCommand
{

    public function buildOptionParser(ConsoleOptionParser $parser): \Cake\Console\ConsoleOptionParser
    {
        return parent::buildOptionParser($parser)
            ->setDescription(__('Indicate controller and action to bake sample template'))
    ...
    {
        $io->out("\n" . "executing Command sample_template");

        $name = $args->getArgument('name') ?? '';
        debug($name);
        $name = $args->getArgument('Controller') ?? '';
        debug($name);
        $name = $args->getArgument('Action') ?? '';
        debug($name);
